...
class _DriverPersonalInfoScreenState extends State<DriverPersonalInfoScreen> {
  ...
  String? _validateFullName(dynamic value) {
  ...
  String? _validateDateOfBirth(dynamic value) {
    final v = (value as String?)?.trim() ?? '';
    if (v.isEmpty) return 'Please select date of birth';
    final parts = v.split('-');
    if (parts.length != 3) return 'Invalid date of birth';
    final y = int.tryParse(parts[0]);
    final m = int.tryParse(parts[1]);
    final d = int.tryParse(parts[2]);
    if (y == null || m == null || d == null) return 'Invalid date of birth';
    if (m < 1 || m > 12 || d < 1 || d > 31) return 'Invalid date of birth';
    final birth = DateTime(y, m, d);
    if (birth.isAfter(DateTime.now()))
      return 'Date of birth cannot be in the future';
    final ageYears = DateTime.now().difference(birth).inDays / 365.25;
  ...
  void _submitProfile() {
    ...
    final c = _driverSetupController;
    if (c.selectedGender.value.isEmpty) {
      showCustomSnackBar('Please select gender');
      return;
    }
    if (c.driverProfileImage.value == null) {
      showCustomSnackBar('Please upload profile photo');
      return;
    }
    if (c.nIdfrontImage.value == null) {
      showCustomSnackBar('Please upload National ID (front)');
      return;
    }
    if (c.nIdbackImage.value == null) {
      showCustomSnackBar('Please upload National ID (back)');
      return;
    }
    if (c.frontImage.value == null) {
      showCustomSnackBar('Please upload driving license (front)');
      return;
    }
    if (c.backImage.value == null) {
      showCustomSnackBar('Please upload driving license (back)');
      return;
    }

    c.setupUserProfile(
      avatar: c.driverProfileImage.value!.path,
      drivingLicenseFront: c.frontImage.value!.path,
      drivingLicenseBack: c.backImage.value!.path,
      nIdFornt: c.nIdfrontImage.value!.path,
      nIdBack: c.nIdbackImage.value!.path,
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            Stack(
                              ...
                              children: [
                                Obx(() {
                                  final image = _driverSetupController
                                      .nIdfrontImage
                                      .value;
                                  return InkWell(
                                    onTap: () {
